...
// Context to share sweepPos and colors with child lines
interface DiaTextRevealContextValue {
  sweepPos: any;
  colors: string[];
}
const DiaTextRevealContext = createContext<DiaTextRevealContextValue | null>(
  null,
);

export interface DiaTextRevealLineProps extends Omit<
  HTMLMotionProps<"span">,
  "ref" | "children" | "style" | "animate" | "transition"
> {
  textColor?: string;
  className?: string;
...
export function DiaTextRevealLine({
  children,
  textColor = "var(--foreground)",
  className,
  ...props
}: DiaTextRevealLineProps) {
  const context = useContext(DiaTextRevealContext);
  ...
  const { sweepPos, colors } = context;

  const backgroundImage = useTransform(sweepPos, (pos: number) =>
...
export const DiaTextReveal = Object.assign(
  function DiaTextReveal({
    ...
    const Component = as === "h1" ? motion.h1 : motion.span;

    if (children) {
      return (
        <DiaTextRevealContext.Provider value={{ sweepPos, colors }}>
          <Component
            ref={spanRef as any}
            className={className}
            {...(props as any)}
